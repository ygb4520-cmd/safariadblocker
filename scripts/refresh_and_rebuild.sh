#!/bin/bash
# Daily maintenance job (invoked by the launchd agent installed via
# scripts/install_weekly_refresh.sh -- filename predates the switch to
# daily): re-downloads EasyList/EasyPrivacy/URLhaus/phishing-filter,
# regenerates rules/{ads,trackers,malware,cosmetic}.json, and rebuilds the
# app so the refreshed rules are packaged in.
#
...
/usr/bin/python3 "$REPO_ROOT/scripts/convert_filterlists.py"
